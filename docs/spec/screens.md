...
2. **Log Match** — singles/doubles toggle, player pickers (logger pre-filled),
   ...
   Payoff moment after submit: rating changes, followed by a `Next match`
   section. For doubles, it offers every valid pairing of the submitted
   participants; for singles, it offers the same participants. Either format
   can instead start the ordinary Logger-only form with different players.
   Choosing a repeat option starts a fresh result-less Match. A copied Guest
...
Every main-tab selection changes the active-tab styling immediately. The tab
labels carry no separate spinner, dot, badge, or other inline pending marker:
prefetched navigations are normally too brief for one to communicate anything,
and reserving room for a hidden marker changes the tab geometry. Feed,
Leaderboard, Player Detail and Edit Match show destination-shaped skeletons
while a server read blocks the route. After five seconds without fresh data,
the Feed or Leaderboard skeleton gives way to the
timestamped Saved View when one exists, or to a retryable connection state
when it does not. A persistent,
...
| 157 | 2026-08-30 | Main-tab pending feedback without inline markers | Selecting Feed, Log Match or Rankings changes the active-tab styling immediately, which is the tab bar's complete per-link acknowledgement. There is no spinner, dot, badge or other inline pending marker beside a tab label: prefetched transitions commonly skip or flash that state, route-level destination loading and recovery surfaces carry meaningful waits, and an always-rendered hidden marker altered the label geometry it was meant to leave stable. This removes the `useLinkStatus` dot added during slow-connection work while retaining optimistic active-tab state. |
| 158 | 2026-08-30 | One skeleton language for server-read waits | Player Detail and Edit Match replace their text-only `Contacting the server…` cards with destination-shaped skeletons from the same shared component as Feed and Rankings. Route-level waits that block on a server read now use one accessible, reduced-motion-aware loading language. Log Match remains the deliberate exception because its snapshot-first form is usable while the live roster refreshes; prolonged writes retain explicit status copy because skeletons would hide the draft or falsely imply cancellation. |
| 159 | 2026-08-30 | Repeat Match launchpad | Every successful online or safely queued new-Match summary ends with a separate `Next match` section; edit confirmations do not. This **reverses** decision 153's direct same-Side/same-slot `Log another match` reset. Doubles offers every domain-valid pairing of the submitted participants, ordered with changed pairings before the prior pairing marked `Same teams`; singles offers the same participants. Choosing an option immediately opens a fresh result-less Match with that lineup, while `Choose different players` opens the ordinary doubles-first Logger-only form. The section always carries its stable `Next match` label plus one uncomplicated random slogan, chosen once when that summary appears from `Same court, new alliances.`, `New teams, fresh excuses.`, `Different players, same problems.`, and `New partners, same bragging rights.`; repeats are allowed and the chosen line stays fixed for that visible summary. Roster Players already occupying another participant slot remain visible in pickers with their current Side indicated, and choosing one swaps the two Players atomically rather than creating a duplicate draft. The feature is a repeat-entry convenience only: it creates no Session, rotation sequence, balance claim, or persistent Guest identity. |
| 160 | 2026-08-30 | Compact Next Match rows | The Next Match launchpad uses full-width gold-outline action rows. Each pairing stacks partner names within two Sides around a small centered `VS`; the submitted pairing carries a restrained `Same teams` marker. `Choose different players` shares the row treatment with a quiet full-roster cue. This keeps every action factual and equally weighted while fitting the four-Player Rating payoff at the supported phone widths. |
